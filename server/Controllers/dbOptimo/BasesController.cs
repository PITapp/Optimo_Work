...
namespace OptimoWork.Controllers.DbOptimo
{
  ...
  [ODataRoutePrefix("odata/dbOptimo/Bases")]
  [Route("mvc/odata/dbOptimo/Bases")]
  public partial class BasesController : ODataController
  {
    private Data.DbOptimoContext context;

    public BasesController(Data.DbOptimoContext context)
    {
      this.context = context;
    }
    // GET /odata/DbOptimo/Bases
    [EnableQuery(MaxExpansionDepth=10,MaxAnyAllExpressionDepth=10,MaxNodeCount=1000)]
    [HttpGet]
    public IEnumerable<Models.DbOptimo.Base> GetBases()
    {
      var items = this.context.Bases.AsQueryable<Models.DbOptimo.Base>();
      this.OnBasesRead(ref items);

      return items;
    }

    partial void OnBasesRead(ref IQueryable<Models.DbOptimo.Base> items);
    ...
    partial void OnBasesGet(ref IQueryable<Models.DbOptimo.Base> items);

    partial void OnBaseDeleted(Models.DbOptimo.Base item);
    ...
    partial void OnBaseUpdated(Models.DbOptimo.Base item);

    [HttpPut("{BaseID}")]
    [EnableQuery(MaxExpansionDepth=10,MaxAnyAllExpressionDepth=10,MaxNodeCount=1000)]
    public IActionResult PutBase(int key, [FromBody]Models.DbOptimo.Base newItem)
    {
        try
        {
            if(!ModelState.IsValid)
    ...
    [HttpPatch("{BaseID}")]
    [EnableQuery(MaxExpansionDepth=10,MaxAnyAllExpressionDepth=10,MaxNodeCount=1000)]
    public IActionResult PatchBase(int key, [FromBody]Delta<Models.DbOptimo.Base> patch)
    {
        try
        {
            if(!ModelState.IsValid)
    ...
    partial void OnBaseCreated(Models.DbOptimo.Base item);

    [HttpPost]
    [EnableQuery(MaxExpansionDepth=10,MaxAnyAllExpressionDepth=10,MaxNodeCount=1000)]
    public IActionResult Post([FromBody] Models.DbOptimo.Base item)
    {
        try
        {
            if(!ModelState.IsValid)
